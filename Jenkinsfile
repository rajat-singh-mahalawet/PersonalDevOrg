    ...
    stage('Authenticate with Salesforce'){
      ...
      steps{
        withCredentials([file(credentialsId: "${SERVER_KEY_CREDENTALS_ID}", variable: 'serverkey_file')]) {
          ...
          bat "\"${toolbelt}\" force:auth:jwt:grant --clientid ${SF_CONSUMER_KEY} --username ${SF_USERNAME} --jwtkeyfile \"${serverkey_file}\" --setdefaultdevhubusername --instanceurl ${SF_INSTANCE_URL}"
